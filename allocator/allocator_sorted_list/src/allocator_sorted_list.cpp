...
// размер 68 байт зарезервирован для метаданных аллокатора в доверенной памяти
...
inline char* get_managed_space_start(void* trusted) { //вычисление конца метаданных и начала управляемого пространства
...
//68 байт на служебные данные + `space_size` для пользователей
...
[[nodiscard]] void *allocator_sorted_list::do_allocate_sm(size_t size)
{
    ...
    while (curr != nullptr) {
        ...
        curr_prev_ptr = reinterpret_cast<void**>(curr);
        curr = meta->next_free;
    }
...
void allocator_sorted_list::do_deallocate_sm(void *at)
{
    ...
    // указатель на trusted memory(проверка моя не моя)
