...
// Metadata layout for the allocator (68 bytes)
...
inline void** get_first_block_ptr(void* trusted) {
    return reinterpret_cast<void**>(reinterpret_cast<char*>(trusted) + 60);
}

struct block_metadata {
    size_t size; // Total size including metadata. LSB is occupancy flag (1=occupied).
    block_metadata* prev;
    block_metadata* next;
    void* allocator_ptr;
};

// block_metadata is 32 bytes.
    ...
    allocator_with_fit_mode::fit_mode allocate_fit_mode)
{
    ...
    *get_total_size_ptr(_trusted_memory) = space_size;
    ...
    if (space_size >= METADATA_SIZE) {
        ...
        first->size = space_size; // Occupied = 0
        first->prev = nullptr;
        first->next = nullptr;
...
// Rule of Five (basic for this lab)
...
[[nodiscard]] void *allocator_boundary_tags::do_allocate_sm(size_t bytes)
{
    ...
    if (get_size(best) >= needed + METADATA_SIZE) {
        ...
        next_block->size = old_size - needed; // is_occupied = 0
...
void allocator_boundary_tags::do_deallocate_sm(void *at)
{
    ...
    if (curr->next && !is_occupied(curr->next)) {
        ...
        curr->size = get_size(curr) + get_size(next); // keep is_occupied = 0
        curr->next = next->next;
    ...
    if (curr->prev && !is_occupied(curr->prev)) {
        ...
        prev->size = get_size(prev) + get_size(curr); // keep is_occupied = 0
        prev->next = curr->next;
